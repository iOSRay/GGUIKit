...
@implementation GGStickyHeaderFlowLayout
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    [noneHeaderSections enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop) {
        [self.sections enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, NSUInteger index, BOOL * _Nonnull stop) {
    ...
    for (UICollectionViewLayoutAttributes *attributes in superArray) {
        ...
        if ([attributes.representedElementKind isEqualToString:UICollectionElementKindSectionHeader]) {
            for (NSNumber *section in self.sections) {
                if (attributes.indexPath.section == section.integerValue) {
                    ...
                    NSIndexPath *lastItemIndexPath = [NSIndexPath indexPathForItem:MAX(0, numberOfItemsInSection) inSection:attributes.indexPath.section];
                    ...
                    CGFloat offset = self.collectionView.contentOffset.y + self.navigationHeight;
                    // 第一个cell的y值 - 当前header的高度 - 可能存在的sectionInset的top
                    CGFloat headerY = firstItemAttributes.frame.origin.y - self.sectionInset.top - rect.size.height;
                    // 哪个大取哪个，保证header悬停
